#include "vlt_hashmap.h"
#include "vctest.h"

#include <stdbool.h>

typedef struct HMapTestInput
{
    const U8* key;
    const U64 keySize;
    U8* val;
    const U64 valSize;
}
HMapTestInput;

TEST(whenNewHMap_thenNewHMapIsCreatedAndHMapObjIsInitializedProperlyAndPointerToNewHMapIsReturned,
     "When new_hmap, then a new HMap obj is created with properly"
     " initialized fields and a pointer to the HMap obj is returned")
{
    HMap* hm = new_hmap();
    VCTEST_ASSERT_TRUE(hm != NULL);
    VCTEST_ASSERT_TRUE(hm->size == 0);

    del_hmap(hm);
    hm = NULL;
}

TEST(whenHMapPut_thenTheValueWithKeyMustBeAddedAndRetrievedByKeyAndHMapObjMustBeUpdatedProperly,
     "When hmap_put, then the given value with the key MUST"
     " be put in the hashmap, retrieved by the key, and the"
     " hashmap object MUST be properly updated")
{
    int testInputsSize = 5;
    HMapTestInput inputs[5] = {
        // TODO: should your hashmap handle empty keys?
        // {
        //     .key = (const U8[]) { },
        //     .keySize = 0,
        //     .val = (U8[]) { },
        //     .valSize = 0,
        // },
        {
            .key = (const U8[]) { 0x00 },
            .keySize = 1,
            .val = (U8[]) { 0xFF },
            .valSize = 1,
        },
        {
            .key = (const U8[]) {
                0x00, 0x01, 0x00, 0xFF, 0x00, 0x7E, 0x00, 0x42
            },
            .keySize = 8,
            .val = (U8[]) {
                0xDE, 0xAD, 0xBE, 0xEF,
                0x00, 0x11, 0x22, 0x33,
                0x44, 0x55, 0x66, 0x77
            },
            .valSize = 12,
        },
        {
            .key = (const U8[]) {
                0x68, 0x61, 0x73, 0x68, 0x6D, 0x61, 0x70
            },
            .keySize = 7,
            .val = (U8[]) {
                0x13, 0x37, 0xCA, 0xFE, 0xBA, 0xBE,
                0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC,
                0xDE, 0xF0, 0x00, 0xFF
            },
            .valSize = 16,
        },
        {
            .key = (const U8[]) {
                0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x80,
                0x90, 0xA0, 0xB0, 0xC0, 0xD0, 0xE0, 0xF0, 0xFF,
                0xEE, 0xDD, 0xCC, 0xBB, 0xAA, 0x99, 0x88, 0x77,
                0x66, 0x55, 0x44, 0x33, 0x22, 0x11
            },
            .keySize = 30,
            .val = (U8[]) {
                0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
                0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF,
                0xFF, 0xEE, 0xDD, 0xCC, 0xBB, 0xAA, 0x99, 0x88,
                0x77, 0x66, 0x55, 0x44, 0x33, 0x22, 0x11, 0x00,
                0xDE, 0xAD, 0xBE, 0xEF, 0xCA, 0xFE, 0xBA, 0xBE,
                0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC, 0xDE, 0xF0
            },
            .valSize = 48,
        },
        {
            .key = (const U8[]) {
                0xA7, 0x3C, 0x91, 0xE2, 0x5B, 0x08, 0xD4,
                0x6F, 0xB9, 0x21, 0xC6, 0x4A, 0xF0, 0x7D
            },
            .keySize = 14,
            .val = (U8[]) {
                0x4E, 0xA1, 0x7B, 0xD3, 0x29, 0xF8, 0x65, 0x0C,
                0xB7, 0x42, 0x9D, 0xE6, 0x13, 0x58, 0xCA, 0x34,
                0x8F, 0xD0, 0x76, 0x2B, 0xA9, 0x45, 0xE1, 0xBC,
                0x07, 0x93, 0x5D, 0xF2, 0x68, 0xC4, 0x1A, 0x87,
                0xDB, 0x30, 0xAE, 0x59, 0xC1, 0x74, 0x0F, 0xE8,
                0x26, 0xB3, 0x4C, 0x95, 0xF7, 0x61, 0xDA, 0x18,
                0x83, 0x2E, 0xC9, 0x50, 0xAB, 0x06, 0xF4, 0x3D,
                0x79, 0xE3, 0x14, 0xBD, 0x67, 0x28, 0x9A, 0xD5,
                0x41, 0xFC, 0x0B, 0x72, 0xC7, 0x35, 0x88, 0xE0,
                0x5A, 0xAF, 0x1D, 0x96, 0x43, 0xCB, 0x70, 0x04,
                0xF1, 0x2A, 0xB8, 0x63, 0xDE, 0x19, 0x85, 0x4F,
                0xA3, 0x57, 0xCC, 0x31, 0x0A, 0xED, 0x69, 0xB4
            },
            .valSize = 96,
        },
    };

    HMap* hm = new_hmap();
    VCTEST_ASSERT_TRUE(hm != NULL);
    U64 hmCurrSize = hm->size;

    for (int i = 0; i < testInputsSize; i++)
    {
        // GIVEN
        const U8* const key = inputs[i].key;
        const U64 keySize = inputs[i].keySize;
        U8* val = inputs[i].val;
        const U64 valSize = inputs[i].valSize;

        // WHEN
        hmap_put(hm, key, keySize, val, valSize);
        hmCurrSize++;

        // THEN
        VCTEST_ASSERT_TRUE(hmCurrSize == hm->size);
        MapElem* me = hmap_get(hm, key, keySize);
        VCTEST_ASSERT_TRUE(me->keySize == keySize);
        VCTEST_ASSERT_TRUE( bytesEqual(me->key, key, keySize) );
        VCTEST_ASSERT_TRUE(me->valSize == valSize);
        VCTEST_ASSERT_TRUE( bytesEqual(me->val, val, valSize) );
        VCTEST_ASSERT_TRUE(me->key != key); // test the key is copied when inserted
        VCTEST_ASSERT_TRUE(me->val != val); // test the value is copied when inserted
    }

    // CLEAN-UP
    del_hmap(hm);
}

TEST(whenHMapGet_thenPointerToExpectedValueBytesWithKeyMustBeReturned,
     "When hmap_get, then a non-NULL pointer to a map element with"
     " expected key and value bytes MUST be returned")
{
    int testInputsSize = 5;
    HMapTestInput inputs[5] = {
        // TODO: should your hashmap handle empty keys?
        // {
        //     .key = (const U8[]) { },
        //     .keySize = 0,
        //     .val = (U8[]) { },
        //     .valSize = 0,
        // },
        {
            .key = (const U8[]) { 0x00 },
            .keySize = 1,
            .val = (U8[]) { 0xFF },
            .valSize = 1,
        },
        {
            .key = (const U8[]) {
                0x00, 0x01, 0x00, 0xFF, 0x00, 0x7E, 0x00, 0x42
            },
            .keySize = 8,
            .val = (U8[]) {
                0xDE, 0xAD, 0xBE, 0xEF,
                0x00, 0x11, 0x22, 0x33,
                0x44, 0x55, 0x66, 0x77
            },
            .valSize = 12,
        },
        {
            .key = (const U8[]) {
                0x68, 0x61, 0x73, 0x68, 0x6D, 0x61, 0x70
            },
            .keySize = 7,
            .val = (U8[]) {
                0x13, 0x37, 0xCA, 0xFE, 0xBA, 0xBE,
                0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC,
                0xDE, 0xF0, 0x00, 0xFF
            },
            .valSize = 16,
        },
        {
            .key = (const U8[]) {
                0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x80,
                0x90, 0xA0, 0xB0, 0xC0, 0xD0, 0xE0, 0xF0, 0xFF,
                0xEE, 0xDD, 0xCC, 0xBB, 0xAA, 0x99, 0x88, 0x77,
                0x66, 0x55, 0x44, 0x33, 0x22, 0x11
            },
            .keySize = 30,
            .val = (U8[]) {
                0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
                0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF,
                0xFF, 0xEE, 0xDD, 0xCC, 0xBB, 0xAA, 0x99, 0x88,
                0x77, 0x66, 0x55, 0x44, 0x33, 0x22, 0x11, 0x00,
                0xDE, 0xAD, 0xBE, 0xEF, 0xCA, 0xFE, 0xBA, 0xBE,
                0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC, 0xDE, 0xF0
            },
            .valSize = 48,
        },
        {
            .key = (const U8[]) {
                0xA7, 0x3C, 0x91, 0xE2, 0x5B, 0x08, 0xD4,
                0x6F, 0xB9, 0x21, 0xC6, 0x4A, 0xF0, 0x7D
            },
            .keySize = 14,
            .val = (U8[]) {
                0x4E, 0xA1, 0x7B, 0xD3, 0x29, 0xF8, 0x65, 0x0C,
                0xB7, 0x42, 0x9D, 0xE6, 0x13, 0x58, 0xCA, 0x34,
                0x8F, 0xD0, 0x76, 0x2B, 0xA9, 0x45, 0xE1, 0xBC,
                0x07, 0x93, 0x5D, 0xF2, 0x68, 0xC4, 0x1A, 0x87,
                0xDB, 0x30, 0xAE, 0x59, 0xC1, 0x74, 0x0F, 0xE8,
                0x26, 0xB3, 0x4C, 0x95, 0xF7, 0x61, 0xDA, 0x18,
                0x83, 0x2E, 0xC9, 0x50, 0xAB, 0x06, 0xF4, 0x3D,
                0x79, 0xE3, 0x14, 0xBD, 0x67, 0x28, 0x9A, 0xD5,
                0x41, 0xFC, 0x0B, 0x72, 0xC7, 0x35, 0x88, 0xE0,
                0x5A, 0xAF, 0x1D, 0x96, 0x43, 0xCB, 0x70, 0x04,
                0xF1, 0x2A, 0xB8, 0x63, 0xDE, 0x19, 0x85, 0x4F,
                0xA3, 0x57, 0xCC, 0x31, 0x0A, 0xED, 0x69, 0xB4
            },
            .valSize = 96,
        },
    };

    HMap* hm = new_hmap();
    VCTEST_ASSERT_TRUE(hm != NULL);

    for (int i = 0; i < testInputsSize; i++)
    {
        // GIVEN
        const U8* const key = inputs[i].key;
        const U64 keySize = inputs[i].keySize;
        U8* val = inputs[i].val;
        const U64 valSize = inputs[i].valSize;
        hmap_put(hm, key, keySize, val, valSize);

        // WHEN
        MapElem* retrievedElem = hmap_get(hm, key, keySize);

        // THEN
        VCTEST_ASSERT_TRUE(retrievedElem != NULL);
        VCTEST_ASSERT_TRUE(retrievedElem->keySize == keySize);
        VCTEST_ASSERT_TRUE( bytesEqual(retrievedElem->key, key, keySize) );
        VCTEST_ASSERT_TRUE(retrievedElem->valSize == valSize);
        VCTEST_ASSERT_TRUE( bytesEqual(retrievedElem->val, val, valSize) );
    }

    // CLEAN-UP
    del_hmap(hm);
}

TEST(whenHMapGet_thenNullPointerMustBeReturned,
     "When hmap_get a key that the hashmap doesn't have, then"
     " NULL pointer MUST be returned")
{
    int testInputsSize = 5;
    HMapTestInput inputs[5] = {
        // TODO: should your hashmap handle empty keys?
        // {
        //     .key = (const U8[]) { },
        //     .keySize = 0,
        //     .val = (U8[]) { },
        //     .valSize = 0,
        // },
        {
            .key = (const U8[]) { 0x00 },
            .keySize = 1,
            .val = (U8[]) { 0xFF },
            .valSize = 1,
        },
        {
            .key = (const U8[]) {
                0x00, 0x01, 0x00, 0xFF, 0x00, 0x7E, 0x00, 0x42
            },
            .keySize = 8,
            .val = (U8[]) {
                0xDE, 0xAD, 0xBE, 0xEF,
                0x00, 0x11, 0x22, 0x33,
                0x44, 0x55, 0x66, 0x77
            },
            .valSize = 12,
        },
        {
            .key = (const U8[]) {
                0x68, 0x61, 0x73, 0x68, 0x6D, 0x61, 0x70
            },
            .keySize = 7,
            .val = (U8[]) {
                0x13, 0x37, 0xCA, 0xFE, 0xBA, 0xBE,
                0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC,
                0xDE, 0xF0, 0x00, 0xFF
            },
            .valSize = 16,
        },
        {
            .key = (const U8[]) {
                0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x80,
                0x90, 0xA0, 0xB0, 0xC0, 0xD0, 0xE0, 0xF0, 0xFF,
                0xEE, 0xDD, 0xCC, 0xBB, 0xAA, 0x99, 0x88, 0x77,
                0x66, 0x55, 0x44, 0x33, 0x22, 0x11
            },
            .keySize = 30,
            .val = (U8[]) {
                0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
                0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF,
                0xFF, 0xEE, 0xDD, 0xCC, 0xBB, 0xAA, 0x99, 0x88,
                0x77, 0x66, 0x55, 0x44, 0x33, 0x22, 0x11, 0x00,
                0xDE, 0xAD, 0xBE, 0xEF, 0xCA, 0xFE, 0xBA, 0xBE,
                0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC, 0xDE, 0xF0
            },
            .valSize = 48,
        },
        {
            .key = (const U8[]) {
                0xA7, 0x3C, 0x91, 0xE2, 0x5B, 0x08, 0xD4,
                0x6F, 0xB9, 0x21, 0xC6, 0x4A, 0xF0, 0x7D
            },
            .keySize = 14,
            .val = (U8[]) {
                0x4E, 0xA1, 0x7B, 0xD3, 0x29, 0xF8, 0x65, 0x0C,
                0xB7, 0x42, 0x9D, 0xE6, 0x13, 0x58, 0xCA, 0x34,
                0x8F, 0xD0, 0x76, 0x2B, 0xA9, 0x45, 0xE1, 0xBC,
                0x07, 0x93, 0x5D, 0xF2, 0x68, 0xC4, 0x1A, 0x87,
                0xDB, 0x30, 0xAE, 0x59, 0xC1, 0x74, 0x0F, 0xE8,
                0x26, 0xB3, 0x4C, 0x95, 0xF7, 0x61, 0xDA, 0x18,
                0x83, 0x2E, 0xC9, 0x50, 0xAB, 0x06, 0xF4, 0x3D,
                0x79, 0xE3, 0x14, 0xBD, 0x67, 0x28, 0x9A, 0xD5,
                0x41, 0xFC, 0x0B, 0x72, 0xC7, 0x35, 0x88, 0xE0,
                0x5A, 0xAF, 0x1D, 0x96, 0x43, 0xCB, 0x70, 0x04,
                0xF1, 0x2A, 0xB8, 0x63, 0xDE, 0x19, 0x85, 0x4F,
                0xA3, 0x57, 0xCC, 0x31, 0x0A, 0xED, 0x69, 0xB4
            },
            .valSize = 96,
        },
    };

    // GIVEN
    HMap* hm = new_hmap();
    VCTEST_ASSERT_TRUE(hm != NULL);

    for (int i = 0; i < testInputsSize; i++)
    {
        // WHEN
        MapElem* retrievedElem = hmap_get(hm, inputs[i].key, inputs[i].keySize);

        // THEN
        VCTEST_ASSERT_TRUE(retrievedElem == NULL);
    }

    const U8* const notInsertedKey = (U8[]) {
        0x4E, 0xA1, 0x7B, 0xD3, 0x29, 0xF8, 0x65, 0x0C,
        0xB7, 0x42, 0x9D, 0xE6, 0x13, 0x58, 0xCA, 0x34,
        0x8F, 0xD0, 0x76, 0x2B, 0xA9, 0x45, 0xE1, 0xBC,
        0x07, 0x93, 0x5D, 0xF2, 0x68, 0xC4, 0x1A, 0x87,
        0xDB, 0x30, 0xAE, 0x59, 0xC1, 0x74, 0x0F, 0xE8,
        0x26, 0xB3, 0x4C, 0x95, 0xF7, 0x61, 0xDA, 0x18,
        0x83, 0x2E, 0xC9, 0x50, 0xAB, 0x06, 0xF4, 0x3D,
        0x79, 0xE3, 0x14, 0xBD, 0x67, 0x28, 0x9A, 0xD5,
        0x41, 0xFC, 0x0B, 0x72, 0xC7, 0x35, 0x88, 0xE0,
        0x5A, 0xAF, 0x1D, 0x96, 0x43, 0xCB, 0x70, 0x04,
        0xF1, 0x2A, 0xB8, 0x63, 0xDE, 0x19, 0x85, 0x4F,
        0xA3, 0x57, 0xCC, 0x31, 0x0A, 0xED, 0x69, 0xB4
    };
    const U64 notInsertedKeySize = 96;

    // GIVEN
    for (int i = 0; i < testInputsSize; i++)
    {
        hmap_put(hm, inputs[i].key, inputs[i].keySize, inputs[i].val, inputs[i].valSize);
        MapElem* retrievedElem = hmap_get(hm, inputs[i].key, inputs[i].keySize);
        VCTEST_ASSERT_TRUE(retrievedElem != NULL);
    }

    // WHEN
    MapElem* retrievedElem = hmap_get(hm, notInsertedKey, notInsertedKeySize);

    // THEN
    VCTEST_ASSERT_TRUE(retrievedElem == NULL);

    // CLEAN-UP
    del_hmap(hm);
}

TEST(whenHMapDelete_thenMapElemWithGivenKeyMustBeDeleted,
     "When hmap_delete, then the map element with the given"
     " key MUST be deleted from the hashmap and hmap_get by"
     " the key MUST return NULL")
{
    int testInputsSize = 5;
    HMapTestInput inputs[5] = {
        // TODO: should your hashmap handle empty keys?
        // {
        //     .key = (const U8[]) { },
        //     .keySize = 0,
        //     .val = (U8[]) { },
        //     .valSize = 0,
        // },
        {
            .key = (const U8[]) { 0x00 },
            .keySize = 1,
            .val = (U8[]) { 0xFF },
            .valSize = 1,
        },
        {
            .key = (const U8[]) {
                0x00, 0x01, 0x00, 0xFF, 0x00, 0x7E, 0x00, 0x42
            },
            .keySize = 8,
            .val = (U8[]) {
                0xDE, 0xAD, 0xBE, 0xEF,
                0x00, 0x11, 0x22, 0x33,
                0x44, 0x55, 0x66, 0x77
            },
            .valSize = 12,
        },
        {
            .key = (const U8[]) {
                0x68, 0x61, 0x73, 0x68, 0x6D, 0x61, 0x70
            },
            .keySize = 7,
            .val = (U8[]) {
                0x13, 0x37, 0xCA, 0xFE, 0xBA, 0xBE,
                0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC,
                0xDE, 0xF0, 0x00, 0xFF
            },
            .valSize = 16,
        },
        {
            .key = (const U8[]) {
                0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x80,
                0x90, 0xA0, 0xB0, 0xC0, 0xD0, 0xE0, 0xF0, 0xFF,
                0xEE, 0xDD, 0xCC, 0xBB, 0xAA, 0x99, 0x88, 0x77,
                0x66, 0x55, 0x44, 0x33, 0x22, 0x11
            },
            .keySize = 30,
            .val = (U8[]) {
                0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
                0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF,
                0xFF, 0xEE, 0xDD, 0xCC, 0xBB, 0xAA, 0x99, 0x88,
                0x77, 0x66, 0x55, 0x44, 0x33, 0x22, 0x11, 0x00,
                0xDE, 0xAD, 0xBE, 0xEF, 0xCA, 0xFE, 0xBA, 0xBE,
                0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC, 0xDE, 0xF0
            },
            .valSize = 48,
        },
        {
            .key = (const U8[]) {
                0xA7, 0x3C, 0x91, 0xE2, 0x5B, 0x08, 0xD4,
                0x6F, 0xB9, 0x21, 0xC6, 0x4A, 0xF0, 0x7D
            },
            .keySize = 14,
            .val = (U8[]) {
                0x4E, 0xA1, 0x7B, 0xD3, 0x29, 0xF8, 0x65, 0x0C,
                0xB7, 0x42, 0x9D, 0xE6, 0x13, 0x58, 0xCA, 0x34,
                0x8F, 0xD0, 0x76, 0x2B, 0xA9, 0x45, 0xE1, 0xBC,
                0x07, 0x93, 0x5D, 0xF2, 0x68, 0xC4, 0x1A, 0x87,
                0xDB, 0x30, 0xAE, 0x59, 0xC1, 0x74, 0x0F, 0xE8,
                0x26, 0xB3, 0x4C, 0x95, 0xF7, 0x61, 0xDA, 0x18,
                0x83, 0x2E, 0xC9, 0x50, 0xAB, 0x06, 0xF4, 0x3D,
                0x79, 0xE3, 0x14, 0xBD, 0x67, 0x28, 0x9A, 0xD5,
                0x41, 0xFC, 0x0B, 0x72, 0xC7, 0x35, 0x88, 0xE0,
                0x5A, 0xAF, 0x1D, 0x96, 0x43, 0xCB, 0x70, 0x04,
                0xF1, 0x2A, 0xB8, 0x63, 0xDE, 0x19, 0x85, 0x4F,
                0xA3, 0x57, 0xCC, 0x31, 0x0A, 0xED, 0x69, 0xB4
            },
            .valSize = 96,
        },
    };

    HMap* hm = new_hmap();
    VCTEST_ASSERT_TRUE(hm != NULL);

    // GIVEN
    for (int i = 0; i < testInputsSize; i++)
    {
        hmap_put(hm, inputs[i].key, inputs[i].keySize, inputs[i].val, inputs[i].valSize);
        VCTEST_ASSERT_TRUE(hmap_get(hm, inputs[i].key, inputs[i].keySize) != NULL);
    }

    for (int i = 0; i < testInputsSize; i++)
    {
        // WHEN
        hmap_del(hm, inputs[i].key, inputs[i].keySize);

        // THEN
        VCTEST_ASSERT_TRUE(hmap_get(hm, inputs[i].key, inputs[i].keySize) == NULL);
    }

    // CLEAN-UP
    del_hmap(hm);
}
